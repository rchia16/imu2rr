...
# Some legacy project modules import ipdb at module level. Do not require it for
# batch runs on clean environments.
try:
    ...
    _sys.modules.setdefault("ipdb", ipdb)

# Optional signal-processing dependencies may be unavailable in a clean validation
# sandbox. The LOOCV loaders used here do not call pywt.cwt; provide a stub so
# importing the project dataloader is still possible. On the real experiment
# environment, install PyWavelets and the real function will be used.
try:
    import pywt  # type: ignore
except Exception:  # pragma: no cover
    import types
    import sys as _sys
    pywt = types.SimpleNamespace(cwt=lambda *a, **k: (_ for _ in ()).throw(ImportError("pywt.cwt called but PyWavelets is not installed")))
    _sys.modules.setdefault("pywt", pywt)

from dataloader import loocv_generator
from rr_jbhi_tslib_source_models_v2 import SOURCE_BASELINES_TSLIB as SOURCE_BASELINES, RRForward, make_model
...
def run_one_model(model_name: str, args, device: torch.device) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for subject, train_loader, val_loader, test_loader in loocv_generator(
        ...
        print(f"[SOURCE-BASELINE] model={model_name} subject={subject}", flush=True)
        ...
        seed_rows = []
        seed_preds = []
        for ens_idx in range(int(args.inception_ensemble) if model_name == "inceptiontime" else 1):
            set_seed(int(args.seed) + ens_idx)
            model = make_model(model_name, in_ch=in_ch, seq_len=seq_len, emb_dim=args.emb_dim)
...
def main() -> None:
    ...
    ap.add_argument("--models", default=DEFAULT_MODELS, help=f"Space-separated from: {' '.join(SOURCE_BASELINES)}")
    ap.add_argument("--patchtst-file", default="PatchTST.py", help="Path to the TSLib/PatchTST Model file supplied as PatchTST.py")
    ap.add_argument("--timesnet-file", default="TimesNet.py", help="Path to the TSLib TimesNet Model file supplied as TimesNet.py")
    ap.add_argument("--subjects", default=DEFAULT_SUBJECTS)
    ap.add_argument("--data-str", default="imu_filt")
    ap.add_argument("--data-dir", default="/projects/BLVMob/imu-rr-seated/Data")
    ap.add_argument("--data-group", default="mr")
    ap.add_argument("--mdl-dir", default="/projects/BLVMob/imu-rr-seated/models")
    ap.add_argument("--out-dir", default="/projects/BLVMob/imu-rr-seated/results/jbhi_source_baselines")
    ...
    ap.add_argument("--inception-ensemble", type=int, default=1, help="Set 5 for the full InceptionTime ensemble protocol.")
    args = ap.parse_args()
    ...
    summary_all = pd.concat(all_summary, ignore_index=True).sort_values("mae_mean")
