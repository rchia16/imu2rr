...
from digitalsignalprocessing import reject_artefact, downsample_imu_window
...
def make_dataset(
    ...
    if input_downsample_hz is not None:
        print("train set ", end=' ') if is_train else print("test set ", end=' ')
        print("downsampling to ", input_downsample_hz)
        x = downsample_imu_window(
            x, orig_fs=IMU_FS, target_fs=input_downsample_hz
        )
    ...
    if include_subject_id:
        ...
        for data in data_list:
            sid = _subject_from_dict(data)
            if not sid:
                raise ValueError(
                    "Cannot attach subject_id because a loaded subject dict is missing "
                    "a subject key. Use load_data(), which adds subject ids."
                )
            subject_id_parts.append(np.full(len(data[data_str]), sid, dtype=object))
...
class LoadDataset(Dataset):
    def __init__(self, x, y=None, cond=None, br=None, tlx=None, subject_id=None, aug_ratio=0.3, preserve_layout: bool = False, return_subject_id: bool = False):
        ...
        if self.subject_id is not None and len(self.subject_id) != self.len:
            raise ValueError(f"subject_id length {len(self.subject_id)} does not match x length {self.len}")

        if aug_ratio > 0:
            if self.return_subject_id:
                raise ValueError("Augmentation with return_subject_id=True is not supported; disable augmentation or keep subject-id logging for eval/meta loaders.")
    ...
    def __getitem__(self, index):
        subject_meta = None
        if self.return_subject_id and self.subject_id is not None:
            subject_meta = {
