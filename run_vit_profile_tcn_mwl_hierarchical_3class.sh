...
build_common_args() {
  ...
  CMD_ARGS=(
    ...
    --hrv-dropout "$HRV_DROPOUT"
    --hrv-normalization "$HRV_NORMALIZATION"
    --hrv-min-valid-ratio "$HRV_MIN_VALID_RATIO"
    --hrv-input-source "$HRV_INPUT_SOURCE"
  )
  ...
  if [ "$USE_HRV_FUSION" = "1" ]; then CMD_ARGS+=(--use-hrv-fusion); fi
  if [ "$SAVE_CONFUSION_MATRICES" = "1" ]; then CMD_ARGS+=(--save-confusion-matrices); fi
  if [ "$SAVE_TTT_DIAGNOSTICS" = "1" ]; then CMD_ARGS+=(--save-ttt-diagnostics); fi
